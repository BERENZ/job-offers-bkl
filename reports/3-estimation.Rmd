...
iter <- 1 ## po bootstrapach
set.seed(iter)
totals_kod2 <- subset(totals, subset = b == iter)
...
totals_boot <- totals %>% filter(b == iter)
...
t1[,komps[k]] <- 1
t2[,komps[k]] <- 0
t1$hat_wolne <- t1$hat_wolne/2
t2$hat_wolne <- t2$hat_wolne/2
totals_cal <- rbind(t1,t2)
totals_cal <- totals_cal %>% mutate(wt = ifelse(komp_techniczne==1, pred*hat_wolne, hat_wolne*2))
tab_totals <- xtabs(wt ~ rok + komp_techniczne, data = totals_cal)
