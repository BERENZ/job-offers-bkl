...
final_data <- readRDS("../data/bkl-final.rds") %>%
  ...
  mutate(zawod2 = ifelse(zawod2 == 95, 96, zawod2),
         ...
         nace = ifelse(nace %in% c("A", "B", "D", "E"), "ABCE", nace),
...
totals <- readRDS("../data/gus-woj-sek.rds") %>%
  filter(kod1 != 6) %>%
  mutate(kod2 = ifelse(kod2 == 95, 96, kod2),
         ...
         sekcja = ifelse(sekcja %in% c("A", "B", "D", "E"), "ABCE", sekcja),
         rok = as.character(rok)) 
```
...
```{r}
vcd::assocstats(xtabs(~komp_indywidualne + kod1 + rok, data = final_data, subset = zeros > 1)) %>% map("cramer")
vcd::assocstats(xtabs(~komp_indywidualne + kod2 + rok, data = final_data, subset = zeros > 1)) %>% map("cramer")
```
...
```{r}
tot_kod1 <- xtabs(wolne~rok + kod1, data = totals, subset = !is.na(woj))
tot_woj <- xtabs(wolne~rok +  woj, data = totals, subset = !is.na(woj))
tot_sekcja <- xtabs(wolne~rok +  sekcja, data = totals, subset = is.na(woj))
```



Linear calibration for zawod1 
```{r}
cal_kod1 <- calibrate(design = des, formula = list(~rok + kod1), population = list(tot_kod1))
cal_woj <- calibrate(design = des, formula = list(~rok + woj), population = list(tot_woj))
cal_sek <- calibrate(design = des, formula = list(~rok + sekcja), population = list(tot_sekcja))
cal_kod1_sek <- rake(design = des, sample.margins  = list(~rok + sekcja, ~rok + kod1),  
                     population.margins = list(tot_sekcja,tot_kod1), control = list(maxit = 50))
summary(weights(cal_kod1))
summary(weights(cal_woj))
summary(weights(cal_sek))
summary(weights(cal_kod1_sek))
```
...
svymean(~komp_techniczne + komp_matematyczne + komp_kulturalne + komp_komputerowe + 
          ...
          komp_indywidualne + komp_fizyczne + komp_dyspozycyjne + komp_biurowe, cal_kod1) %>% 
  as.data.frame() %>%
  rownames_to_column("komp") %>%
  select(-SE) %>%
  rename(cal_kod1 = mean) %>%
  left_join(
    svymean(~komp_techniczne + komp_matematyczne + komp_kulturalne + komp_komputerowe + 
          komp_kognitywne + komp_kierownicze + komp_interpersonalne + 
          komp_indywidualne + komp_fizyczne + komp_dyspozycyjne + komp_biurowe, cal_woj) %>% 
  as.data.frame() %>%
  rownames_to_column("komp") %>%
  select(-SE) %>%
  rename(cal_woj = mean)
  ...
  left_join(
    svymean(~komp_techniczne + komp_matematyczne + komp_kulturalne + komp_komputerowe + 
          komp_kognitywne + komp_kierownicze + komp_interpersonalne + 
          komp_indywidualne + komp_fizyczne + komp_dyspozycyjne + komp_biurowe, cal_sek) %>% 
  as.data.frame() %>%
  rownames_to_column("komp") %>%
  select(-SE) %>%
  rename(cal_sek = mean)
  ...
  left_join(
    svymean(~komp_techniczne + komp_matematyczne + komp_kulturalne + komp_komputerowe + 
          komp_kognitywne + komp_kierownicze + komp_interpersonalne + 
          komp_indywidualne + komp_fizyczne + komp_dyspozycyjne + komp_biurowe, cal_kod1_sek) %>% 
  as.data.frame() %>%
  rownames_to_column("komp") %>%
  select(-SE) %>%
  rename(cal_kod1_sek = mean)
  ) %>% 
  left_join(
    svymean(~komp_techniczne + komp_matematyczne + komp_kulturalne + komp_komputerowe + 
          komp_kognitywne + komp_kierownicze + komp_interpersonalne + 
...
```{r}
final_data_kod2 <- final_data %>% filter(zeros > 1)
dim(final_data)
dim(final_data %>% filter(zeros > 1))
```

```{r}
des_kod2 <- svydesign(ids = ~1, data = final_data %>% filter(zeros > 1))
svytable(~rok + kod2, des_kod2)
```
```{r}
totals_kod2 <- subset(totals, subset = is.na(woj) & rok != 2012)
tot_kod2 <- xtabs(wolne~rok + kod2, data = totals, subset = !is.na(woj) & rok != 2012)
tot_sek <- xtabs(wolne~rok + sekcja, data = totals, subset = is.na(woj) & rok != 2012)
```

```{r}
cal_kod2 <- calibrate(design = des_kod2, formula = list(~rok + kod2), population = list(tot_kod2))
...
summary(weights(cal_kod2))
summary(weights(cal_kod2_sek))
```
...
for (i in 1:11) {
  ...
  m1 <- mycv.glmnet(x = X, y = final_data_kod2[,komps[i]], intercept = T)
  ...
  ## ridge ada kod
  X <- Matrix::fac2sparse(final_data_kod2$kod2) %>% t() 
  m1 <- mycv.glmnet(x = X, y = final_data_kod2[,komps[i]], intercept = T, alpha = 0)
  m1 <- mycv.glmnet(x = X, y = final_data_kod2[,komps[i]], intercept = T, penalty.factor = 1/abs(m1$coef[-1]))
  X_tot <- Matrix::fac2sparse(totals_kod2$kod2) %>% t() 
  lin_pred  <- as.numeric(cbind(1, X_tot) %*% m1$coef)
  prob_pred <- exp(lin_pred) / (1+exp(lin_pred))
  ...
  X <- cbind(Xkod, Xsek)
  m2 <- mycv.glmnet(x = X, y = final_data_kod2[,komps[i]], intercept = T)
  ...
  Xsek <- Matrix::fac2sparse(final_data_kod2$sekcja) %>% t() 
  X <- cbind(Xkod, Xsek)
  m2 <- mycv.glmnet(x = X, y = final_data_kod2[,komps[i]], intercept = T, alpha = 0)
  m2 <- mycv.glmnet(x = X, y = final_data_kod2[,komps[i]], intercept = T, penalty.factor = 1/abs(m2$coef[-1]))
  Xkod_tot <- Matrix::fac2sparse(totals_kod2$kod2) %>% t() 
  Xsek_tot <- Matrix::fac2sparse(totals_kod2$sekcja) %>% t() 
  X_tot <- cbind(Xkod_tot, Xsek_tot)
  lin_pred  <- as.numeric(cbind(1, X_tot) %*% m2$coef)
